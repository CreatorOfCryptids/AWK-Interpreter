...
public class Parser {
    ...
    boolean parseFunction(ProgramNode node) throws Exception {
        ...
        if (h.matchAndRemove(Token.Type.FUNCTION).isEmpty()) {
            ...
            if (h.matchAndRemove(Token.Type.LPAREN).isEmpty())
                throw new Exception(
                        "Expected a '(' after the " + name + " function declaration at " + h.getErrorPosition());
    ...
    private BlockNode parseBlock() throws Exception {
        // Take in the operation. If there was a Begin or END statement, this will just
    ...
    private Optional<IfNode> parseIf() throws Exception {
        ...
        // Initialize the IfNode pointer outside so we can decide whether to add else
        // statements.
        IfNode returnIfNode;
        ...
        if (h.matchAndRemove(Token.Type.ELSE).isPresent()) {
            ...
            // If there is an if statement after the else, then pass the recursivly called
            // parseIf() to the else if constructor.
            if (h.matchAndRemove(Token.Type.IF).isPresent())
                returnIfNode = new IfNode(condition, statements, parseIf().get());

            // If there is not an if, then just take it as a list of statements and pass
            // that to the else constructor
            else
    ...
    private Optional<StatementNode> parseFunctionCall() throws Exception {
        ...
        if (h.matchAndRemove(Token.Type.PRINT).isPresent()) {
            ...
            // This should not be replaced with parseStatements, because this takes
            // parameters, not statements.
            while (h.moreTokens() && h.matchAndRemove(Token.Type.RPAREN).isEmpty()) {
                parameters.add(parseOperation().get());
                // If there isn't a comma after a parameter, then the next token should be an
                // RPAREN.
    ...
    private Optional<Node> parseAssignment() throws Exception {
        // parseTernary will either return a Ternary node or a node from a lower level,
        // so
        ...
        if (h.matchAndRemove(Token.Type.EXPONENTEQUALS).isPresent()) {
            AssignmentNode retval = new AssignmentNode(left.get(),
                    new OperationNode(left.get(), OperationNode.Operation.EXPONENT, parseMathExpression().get()));
            return Optional.of(retval);
        } else if (h.matchAndRemove(Token.Type.MODEQUALS).isPresent()) {
            AssignmentNode retval = new AssignmentNode(left.get(),
                    new OperationNode(left.get(), OperationNode.Operation.MODULO, parseMathExpression().get()));
            return Optional.of(retval);
        } else if (h.matchAndRemove(Token.Type.TIMESEQUALS).isPresent()) {
            AssignmentNode retval = new AssignmentNode(left.get(),
                    new OperationNode(left.get(), OperationNode.Operation.MULTIPLY, parseMathExpression().get()));
            return Optional.of(retval);
        } else if (h.matchAndRemove(Token.Type.DIVIDEEQUALS).isPresent()) {
            AssignmentNode retval = new AssignmentNode(left.get(),
                    new OperationNode(left.get(), OperationNode.Operation.DIVIDE, parseMathExpression().get()));
            return Optional.of(retval);
        } else if (h.matchAndRemove(Token.Type.PLUSEQUALS).isPresent()) {
            AssignmentNode retval = new AssignmentNode(left.get(),
                    new OperationNode(left.get(), OperationNode.Operation.ADD, parseMathExpression().get()));
            return Optional.of(retval);
        } else if (h.matchAndRemove(Token.Type.MINUSEQUALS).isPresent()) {
            AssignmentNode retval = new AssignmentNode(left.get(),
                    new OperationNode(left.get(), OperationNode.Operation.SUBTRACT, parseMathExpression().get()));
            return Optional.of(retval);
        } else if (h.matchAndRemove(Token.Type.EQUALS).isPresent()) {
            AssignmentNode retval = new AssignmentNode(left.get(), parseTernary().get());
            return Optional.of(retval);
        } else if (h.moreTokens() && (h.peek().get().getType() == Token.Type.STRINGLITERAL
                || h.peek().get().getType() == Token.Type.WORD)) {
            OperationNode retval = new OperationNode(left.get(), OperationNode.Operation.CONCATENATION,
                    parseTernary().get());
            return Optional.of(retval);
        }
     ...
     * 
     * @return A TernaryNode or the next layer down if it doesn't follow the ternary
     *         pattern.
     * @throws Exception
     */
    private Optional<Node> parseTernary() throws Exception {
        ...
        // If there is no question mark, then it doesn't follow the pattern. Return the
        // next layer down.
    ...
    private Optional<Node> parseOrLogic() throws Exception {
        ...
        // If it follows the pattern, make an OR node, if not, return the next layer
        // down.
     ...
     * 
     * @return Returns an OperationNode with an AND operator, or the results from
     *         the next layer down.
     * @throws Exception
     */
     ...
     * 
     * @return Returns an OperationNode with a found boolean Expression, or the
     *         results from the next layer down.
     * @throws Exception
     */
    private Optional<Node> parseBoolean() throws Exception {
        ...
        if (h.matchAndRemove(Token.Type.GREATER).isPresent()) {
            ...
            OperationNode retval = new OperationNode(left.get(), OperationNode.Operation.MATCH,
                    parseBottomLevel().get());
            return Optional.of(retval);
        } else if (h.matchAndRemove(Token.Type.NOTMATCH).isPresent()) {
            OperationNode retval = new OperationNode(left.get(), OperationNode.Operation.NOTMATCH,
                    parseBottomLevel().get());
            return Optional.of(retval);
        } else
            return left;
     ...
     * 
     * @return An OperationNode containing an add or subtract operator, or the
     *         results from the next layer down.
     * @throws Exception
     */
    private Optional<Node> parseMathExpression() throws Exception {
        Optional<Node> retval = parseMathTerm();
        // Repeat catching '+' and '-' to make sure that you catch multiple terms in a
        // row.
        while (h.moreTokens()
                && (h.peek().get().getType() == Token.Type.PLUS || h.peek().get().getType() == Token.Type.MINUS)) {
            if (h.matchAndRemove(Token.Type.PLUS).isPresent()) {
                OperationNode retnode = new OperationNode(retval.get(), OperationNode.Operation.ADD,
                        parseMathTerm().get());
                retval = Optional.of(retnode);
                // return Optional.of(retval);
            } else if (h.matchAndRemove(Token.Type.MINUS).isPresent()) {
                /*
                 * var retval = new OperationNode(left.get(), OperationNode.Operation.SUBTRACT,
                 * parseMathTerm().get());
                 * return Optional.of(retval);
                 */
                OperationNode retnode = new OperationNode(retval.get(), OperationNode.Operation.SUBTRACT,
                        parseMathTerm().get());
                retval = Optional.of(retnode);
            }
        }
     ...
     * 
     * @return an OperationNode containing a multiplication, division, or modulo
     *         operator, or the results from the next layer down.
     * @throws Exception
     */
     ...
     * 
     * @return an OperationNode containing an exponent operator, or the results from
     *         the next layer down.
     * @throws Exception
     */
    private Optional<Node> parseMathPower() throws Exception {
        ...
        if (h.matchAndRemove(Token.Type.EXPONENT).isPresent()) {
            // We call parseMathPower() recursively here because exponents are right
            // associative.
     ...
     * 
     * @return A ConstantNode containing a detected string or number; a PatternNode
     *         containting a detected pattern;
     *         or an OperationNode contatining a detected operation or urinary
     *         operator
     * @throws Exception
     */
    private Optional<Node> parseBottomLevel() throws Exception {
        // Since we need to make sure they're the right type before we consume them, we
        // use peek() instead of /
        ...
        // The nested functions are a bit different in that we are calling parse
        // operation (kinda) recursivly
            ...
            // Don't need to put this one in an Optional because parseOperation already does
            // that for us.
            ...
            retval = new PrePostIteratorNode(temp, OperationNode.Operation.PREINC);
            // retval = new AssignmentNode(temp, retval);
        } else if (h.matchAndRemove(Token.Type.MINUSMINUS).isPresent()) {
            Node temp = parseBottomLevel().get();
            retval = new PrePostIteratorNode(temp, OperationNode.Operation.PREDEC);
            // retval = new AssignmentNode(temp, retval);
        ...
        if (h.matchAndRemove(Token.Type.PLUSPLUS).isPresent()) {
            retval = new PrePostIteratorNode(retval, OperationNode.Operation.POSTINC);
            // retval = new AssignmentNode(retval, temp);
        } else if (h.matchAndRemove(Token.Type.MINUSMINUS).isPresent()) {
            retval = new PrePostIteratorNode(retval, OperationNode.Operation.POSTDEC);
            // retval = new AssignmentNode(retval, temp);
        }
    ...
    private boolean isFunctionCall() {
         ...
         * The goal is to determine if it follows the form of either a user defined
         * function or a built in function.
         * h.moreTokens() && // Make sure that peeking won't return empty and cause an
         * error.
         * (
         * (// Check for user made functions
         * h.peek(1).isPresent() && // Make sure that the next two tokens exist for
         * peeking.
         * (
         * h.peek().get().getType() == Token.Type.WORD &&
         * // Checking for the parentesis because if its a word by itself, then it
         * shouldn't be parsed as a function.
     ...
     * 
     * @return A VariableReferenceNode containing a valid variable, array variable,
     *         or the outcome of a $ operator.
     * @throws Exception
     */
